...
旧环境 `cloud1-d8gp843lt5454ada7` 已销毁；甲方提供的新环境 ID 为 `cloud1-d8gcnzmltc1113cd7`。本地配置已改为新 ID，但账号归属、环境权限、云函数、数据库、静态托管及小程序运行时均尚未在新环境验收。本文件保留重建步骤，以下提到旧环境已完成的初始化和部署均仅为历史记录，不能直接套用到新环境。
